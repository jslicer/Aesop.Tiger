 ...
 * Copyright (C) 2005  Always Elucidated Solution Pioneers, LLC
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 2 of the License, or
 * (at your option) any later version.  This can be found in gpl.txt in
 * the archive this file is shipped with.
 *
 ...
 *
 *    Aesop.Tiger version 1.0.2.0, Copyright (C) 2005 Always Elucidated Solution Pioneers, LLC
 *    Aesop.Tiger comes with ABSOLUTELY NO WARRANTY.
 *    This is free software, and you are welcome to redistribute it
 *    under certain conditions.
*/
namespace Aesop.Tiger
{
    public class Tiger128 : Tiger160
    {
        public new const int HashSizeInBytes = 16;

        public Tiger128() :base()
        {
        }

        public Tiger128(int passes) :base(passes)
        {
        }
        ...
        {
            const string  TEST_DATA = "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq";
            byte[]        TEST_HASH = new byte[HashSizeInBytes] {
                                                                    0xb7, 0x61, 0x0d, 0xf7,
                                                                    0xe8, 0x4f, 0x0a, 0xc3,
                                                                    0xa7, 0x1c, 0x63, 0x1e,
                                                                    0x7b, 0x53, 0xf7, 0x8e
            ...
            Initialize();
            hash                    = ComputeHash(enc.GetBytes(TEST_DATA));
            ...
            for (int nI = 0; nI < TEST_HASH.Length; nI++)
            {
    ...
    public class Tiger160 : Tiger192
    {
        public new const int HashSizeInBytes = 20;

        public Tiger160() :base()
        {
        }

        public Tiger160(int passes) :base(passes)
        {
        }
    ...
    {
        public Tiger192() :base()
        {
        }

        public Tiger192(int passes) :base(passes)
        {
        }
    ...
    public abstract class TigerFull : HashAlgorithm
    {
        ...
        private ulong      c;
        private ulong      cc;
        private int        nbyteBufferPos;
        private byte[]     byteBuffer        = new byte[BlockSizeInBytes];
        private ulong[]    ulongBuffer       = new ulong[BlockSizeInUlongs];

        protected TigerFull()
        {
        }

        protected TigerFull(int passes)
        {
            m_passes = passes;
        }
        ...
        public int Passes
        {
            ...
            set
            {
                if ((value != m_passes) && (value >= DefaultPasses))
                {
                    m_passes = value;
                }
            }
        ...
        {
            int nEnd            = ibStart + cbSize;
            int nToCopy         = BlockSizeInBytes - nbyteBufferPos;
            ...
            while (ibStart < nEnd)
            {
                ...
                Array.Copy(array, ibStart, byteBuffer, nbyteBufferPos,
                    nToCopy);
                ibStart        += nToCopy;
                cbSize         -= nToCopy;
                nbyteBufferPos += nToCopy;
                if (nbyteBufferPos < BlockSizeInBytes)
                {
                    return;
                }
                ProcessBlock(ref a, ref b, ref c, byteBuffer, ulongBuffer);
                nbyteBufferPos  = 0;
                nToCopy         = BlockSizeInBytes;
            }
        }

        protected override byte[] HashFinal()
        {
            byteBuffer[nbyteBufferPos] = 1;
            nbyteBufferPos++;
            if (nbyteBufferPos >= (BlockSizeInBytes - 8))
            {
                Array.Clear(byteBuffer, nbyteBufferPos,
                    BlockSizeInBytes - nbyteBufferPos);
                ProcessBlock(ref a, ref b, ref c, byteBuffer, ulongBuffer);
                nbyteBufferPos         = 0;
            }
            Array.Clear(byteBuffer, nbyteBufferPos,
                (BlockSizeInBytes - nbyteBufferPos) - 8);
            LongToBytes(ulLen << 3, byteBuffer, BlockSizeInBytes - 8);

            ProcessBlock(ref a, ref b, ref c, byteBuffer, ulongBuffer);

            byte[] result              = new byte[HashSizeInBytes];

            LongToBytes(a, result,  0);
            LongToBytes(b, result,  8);
            LongToBytes(c, result, 16);
            return result;
        }

        private void ProcessBlock(ref ulong ap, ref ulong bp, ref ulong cp,
            byte[] byteBuffer1, ulong[] ulongBuffer1)
        {
            int nPos             = 0;
            int nI               = 0;

            while (nPos < BlockSizeInBytes)
            {
                ulongBuffer[nI++] =
                     (((ulong)(byteBuffer1[nPos++])  & 0xff))       |
                    ((((ulong)(byteBuffer1[nPos++])) & 0xff) <<  8) |
                    ((((ulong)(byteBuffer1[nPos++])) & 0xff) << 16) |
                    ((((ulong)(byteBuffer1[nPos++])) & 0xff) << 24) |
                    ((((ulong)(byteBuffer1[nPos++])) & 0xff) << 32) |
                    ((((ulong)(byteBuffer1[nPos++])) & 0xff) << 40) |
                    ((((ulong)(byteBuffer1[nPos++])) & 0xff) << 48) |
                     (((ulong)(byteBuffer1[nPos++]))         << 56);
            }
            compress(ref ap, ref bp, ref cp, ulongBuffer1);
        }

        private void compress(ref ulong ac, ref ulong bc, ref ulong cc1,
            ulong[] ulongBuffer1)
        {
            save_abc(ac, bc, cc1);
            pass(ref ac, ref bc, ref cc1, ulongBuffer1, 5);
            key_schedule(ulongBuffer1);
            pass(ref cc1, ref ac, ref bc, ulongBuffer1, 7);
            key_schedule(ulongBuffer1);
            pass(ref bc, ref cc1, ref ac, ulongBuffer1, 9);

            ulong tmpa;
            int pass_no = DefaultPasses;

            while (pass_no < m_passes)
            {
                pass(ref ac, ref bc, ref cc1, ulongBuffer1, 9);
                tmpa    = ac;
                ac      = cc1;
                cc1     = bc;
                bc      = tmpa;
        ...
        //Candidate #4 for optimization
        private void save_abc(ulong av, ulong bv, ulong cv)
        {
            aa = av;
            bb = bv;
            cc = cv;
        ...
        private static void pass(ref ulong ap, ref ulong bp, ref ulong cp,
            ulong[] ulongBuffer, ulong multiplier)
        {
            round(ref ap, ref bp, ref cp, ulongBuffer[0], multiplier);
            round(ref bp, ref cp, ref ap, ulongBuffer[1], multiplier);
            round(ref cp, ref ap, ref bp, ulongBuffer[2], multiplier);
            round(ref ap, ref bp, ref cp, ulongBuffer[3], multiplier);
            round(ref bp, ref cp, ref ap, ulongBuffer[4], multiplier);
            round(ref cp, ref ap, ref bp, ulongBuffer[5], multiplier);
            round(ref ap, ref bp, ref cp, ulongBuffer[6], multiplier);
            round(ref bp, ref cp, ref ap, ulongBuffer[7], multiplier);
        }

        //Candidate #2 for optimization
        private static void key_schedule(ulong[] ulongBuffer)
        {
        ...
        //Candidate #3 for optimization
        private void feedforward(ref ulong af, ref ulong bf, ref ulong cf)
        {
            af ^= aa;
            bf -= bb;
            cf += cc;
        ...
        //Candidate #1 for optimization
        private static void round(ref ulong ar, ref ulong br, ref ulong cr,
            ulong ulongBuffer, ulong multiplier)
        {
            cr ^= ulongBuffer;
            ar -= (((table1[ (cr        & 0xff)]  ^
        ...
        private static void LongToBytes(ulong ulVal, byte[] byteBuffer,
            int nIdxBuffer)
        {
            const int bytesInALong         = 8;
            const int bitsInAByte          = 8;
            int nEnd                       = nIdxBuffer + bytesInALong;

            while (nIdxBuffer < nEnd)
            {
        ...
        private static ulong[] table1 = new ulong[256] {
        ...
        private static ulong[] table2 = new ulong[256] {
                                                           ...
                                                           0xd0c1e96a19d9e726, 0x5a786a9b4ba9500c, 0xe0020336634c43f3, 0xc17b474aeb66d822,
        ...
        private static ulong[] table3 = new ulong[256] {
        ...
        private static ulong[] table4 = new ulong[256] {
